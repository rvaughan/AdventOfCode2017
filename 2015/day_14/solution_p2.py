#!/usr/bin/env python
"""
This code holds the solution for part 2 of day 14 of the Advent of Code for 2015.
"""
import itertools
import sys
...
for line in test_input.split('\n'):
    reindeer = create_reindeer(line)
    reindeer_map[reindeer.getName()] = reindeer
    score_map[reindeer.getName()] = 0

for _ in xrange(1000):
    max_distance = 0
    lead_reindeers = []
    for name in reindeer_map.keys():
        reindeer_map[name].move_by(1)

    for name in reindeer_map.keys():
        if max_distance < reindeer_map[name].getCurrentDistance():
            max_distance = reindeer_map[name].getCurrentDistance()
            lead_reindeers = [name]
        elif max_distance == reindeer_map[name].getCurrentDistance():
            lead_reindeers.append(name)

    for name in lead_reindeers:
        score_map[name] += 1

max_score = 0
for name in score_map.keys():
    max_score = max(max_score, score_map[name])

assert max_score == 689
...
with open("input.txt", "r") as f:
    ...
    print "Furthest: {}".format(max_score)
