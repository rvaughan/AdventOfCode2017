...
def calc_presents(num_houses=10, num_presents=10, expected_value=0):
    ...
    if expected_value > 0:
        for elf in elves:
            if (num_houses // elf) < 50:
                total += elf * num_presents
...
for house_num in xrange(10):
    houses[house_num] = calc_presents(house_num, 10, 99999)
...
with open("input.txt", "r") as f:
    ...
    # We know that this answer will be bigger than the solution for part 1, so
    # we can start from there...
    start_point = 786240

    houses = {}
    for house_num in xrange(start_point, 1000000):
        houses[house_num] = calc_presents(house_num, 11, 34000000)
