#!/usr/bin/env python
"""
This code holds the solution for part 2 of day 6 of the Advent of Code for 2015.
"""
import hashlib
import sys
...
def process_commands(grid, command):
    ...
    if parts[0] == 'toggle':
        ...
        if parts[1] == 'on':
            set_lights(grid, start_pos, end_pos, 1)
    
        if parts[1] == 'off':
            set_lights(grid, start_pos, end_pos, -1)

    return calc_brightness(grid)
...
light_grid = create_light_grid(0)

run_test("turn on 0,0 through 0,0", 1, light_grid)
run_test("turn off 0,0 through 0,0", 0, light_grid)
run_test("turn off 0,0 through 0,0", 0, light_grid)
run_test("turn on 0,0 through 0,0", 1, light_grid)
run_test("turn on 0,0 through 0,0", 2, light_grid)
run_test("toggle 0,0 through 999,999", 2000002, light_grid)
...
with open("input.txt", "r") as f:
    ...
    print "Brightness: {}".format(result)
