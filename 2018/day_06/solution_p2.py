#!/usr/bin/env python
"""
This code holds the solution for part 2 of day 6 of the Advent of Code for 2018.
"""
...
def check_grid_cell_for_distance(coords, x, y, max_distance):
    result = 0
    ...
    for c_val in coords:
        ...
        result += abs(x - c[0]) + abs(y - c[1])

    return result < max_distance


def find_distance(coords, max_distance):
    count = 0

    for x in xrange(len(grid)):
        for y in xrange(len(grid[0]) - 1):
            if check_grid_cell_for_distance(coords, x, y, max_distance):
                count += 1
...
result = check_grid_cell_for_distance(coords, 4, 3, 32)
assert True == result, "Wrong result at 4, 3, got %d not %d" % (result, True)

result = find_distance(coords, 32)
assert 16 == result, "Wrong distance, got %d not %d" % (result, 16)
...
with open("input.txt", "r") as f:
    ...
    result = find_distance(coords, 10000)

    print "Solution is size: {}".format(result)
