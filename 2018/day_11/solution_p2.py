#!/usr/bin/env python
"""
This code holds the solution for part 2 of day 11 of the Advent of Code for 2018.
"""


def calc_total_power(grid, start_x, start_y, size=3, verbose=False):
    total_power = 0

    for y in xrange(start_y, start_y + size):
        for x in xrange(start_x, start_x + size):
            if verbose:
                print x, y, grid[y][x]
...
def find_max_square(grid):
    ...
    for size in xrange(len(grid)):
        print size
        for y in xrange(len(grid) - (size - 1)):
            for x in xrange(len(grid) - (size - 1)):
                power = calc_total_power(grid, x, y, size)
                if power > max_power:
                    max_power = power
                    max_x = x
                    max_y = y
...
# fuel_grid = new_fuel_grid(300, 300, 18)
# max_x, max_y, max_size, max_power = find_max_square(fuel_grid)
# assert max_x == 90, "{}, {}, {} - {}".format(max_x, max_y, max_size, max_power)
# assert max_y == 269, "{}, {}, {} - {}".format(max_x, max_y, max_size, max_power)
# assert max_power == 113, "{}, {}, {} - {}".format(max_x, max_y, max_size, max_power)
# assert max_power == 16, "{}, {}, {} - {}".format(max_x, max_y, max_size, max_power)
...
with open("input.txt", "r") as f:
    ...
    max_x, max_y, max_power, max_size = find_max_square(fuel_grid)

    print "Solution is {},{},{} - {}".format(max_x, max_y, max_size, max_power)
