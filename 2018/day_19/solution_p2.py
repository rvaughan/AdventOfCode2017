...
with open("input.txt", "r") as f:
    ...
    while ip < len(instructions):
        ip = load_instruction(instructions, ip, registers, initial_ip=init_ip, verbose=True)

        if ip == 1:
            # When ip = 1 we are 'restarting' the program, register 2 seems
            # to contain the product the program has calculated, and we are
            # looking for a clean product of factors.
            print "Solution: {}".format(sum([x for x in xrange(1, registers[2]+1) if registers[2] % x == 0]))
            break
