#!/usr/bin/env python
"""
This code holds the solution for part 2 of day 18 of the Advent of Code for 2018.
"""
...
with open("input.txt", "r") as f:
    ...
    score = 0
    last_score = 0
    for minute in xrange(1, 1000000000):
        do_tick(forest)
        score = calc_score(forest)
        # diff = score - last_score
        # last_score = score

        # print "{:04} {:10} {:6}".format(minute, score, diff)

        if score == 177004:
            print minute, score

        if score not in scores:
            scores[score] = set([minute])
        else:
            # We've found where the pattern starts to repeat, so we can exit
            # print score, scores[score]
            scores[score].add(minute)
            xx = 1000000000 % minute
            for idx in xrange(len(scores[score])):
                if idx % score == xx:
                    period = minute
                    break
