#!/usr/bin/env python
"""
This code holds the solution for part 1 of day 17 of the Advent of Code for 2020.
"""
import re
import sys
...
def step(grid, num):
    ...
    new_grid = [[['.' for k in range(num)] for j in range(num)] for i in range(num)]

    for i in range(1, num-1):
        for j in range(1, num-1):
            for k in range(1, num-1):
                act = find_active(grid, i, j, k)
...
def calculate_solution(grid_data):
    ...
    grid = [[['.' for k in range(grid_size)] for j in range(grid_size)] for i in range(grid_size)]
    for i in range(len(grid_data)):
        for j in range(len(grid_data[i])):
            grid[offset][i+offset][j+offset] = grid_data[i][j]

    for _ in range(0, cycles):
        grid = step(grid, grid_size)

        # for x in grid[offset+0]:
        #     print("".join(x))
        # boop
