...
# This is a solution that I messed up by misunderstand what was being asked for. I thought
# that we were being asked to find the seats at the end of the diagonals, and not just the
# next seat - ignoring floor. *sigh*

edge_seats = {}

def find_edge(row, col, direction, width, height):
    ...
    if key not in edge_seats:
        ...
        if direction == 1:
            ...
            # Right-Up-Diagonal
            while new_row > 0 and new_col < width - 1:
                new_row -= 1
                new_col += 1
        elif direction == 4:
            # Left
            ...
            # Left-down-diagonal
            while new_row < height - 1 and new_col > 0:
                new_row += 1
                new_col -= 1
        elif direction == 7:
            # Down
            ...
            # Right-Down-Diagonal
            while new_row < height - 1 and new_col < width - 1:
                new_row += 1
                new_col += 1
...
def check_seat(seat_map, row, col):
    ...
    for direction in range(1, 9):
        seat_pos = find_edge(row, col, direction, len(seat_map[0]), len(seat_map))
...
def calculate_solution(seat_map):
    ...
    while True:
        # if iteration == 1:
        #     print('------->')
        #     print(new_seat_map[0][3])
        #     print(check_seat(new_seat_map, 0, 3))
        #     print('<-------')
        #     boop
